	...
	for {
		...
		if string(tag) == "table" && t == html.StartTagToken {
			for !((string(tag) == "table" && t == html.EndTagToken) || t == html.ErrorToken) {
				...
				if err = tagLint(depth, tag, t); err != nil {
					return err
				}
				...
				if depth == 0 && t == html.TextToken && len(strings.TrimSpace(string(token.Raw()))) > 0 {
					return fmt.Errorf("line %d illegal character between two tag %s", lintStatus.RowCount, string(token.Raw()))
				}
...
func tagLint(depth int, tag []byte, t html.TokenType) error {
	...
	if !(string(tag) == "th" || string(tag) == "tr" || string(tag) == "td" || string(tag) == "table" || t == html.TextToken) {
		return fmt.Errorf("line %d exist invalid tag: %s", lintStatus.RowCount, string(tag))
	}
	// tag end lint, e.g., ...<tr><th>a</tr>...
	if depth > 2 {
		return fmt.Errorf("line %d miss end tag: </%s>", lintStatus.RowCount, string(tag))
	}
	// tag start lint, e.g., ...<tr>a</th></tr>...
	if depth < 0 {
		return fmt.Errorf("line %d miss start tag: </%s>", lintStatus.RowCount, string(tag))
	}
	return nil
}
